...
- **`latest_tasting_note_date` (Phase 12) is derived, not a column.** Every wine read (`getWine`, `listWines`) computes it with a correlated subquery against the `tasting_notes` row `latest_tasting_note_id` points at. It is excluded from `CreateWineInput`/`UpdateWineInput` and stripped from PATCH bodies. Don't add a column or a migration for it; don't JOIN `tasting_notes` into `listWines` either — the two tables share column names (`my_rating`, `id`) that `listWines`' bare-name WHERE clauses would make ambiguous. `updateWine` re-reads after writing so the derived field is never stale.
- **`POST /api/wines/duplicate-check` (Phase 12)** runs `findDuplicate` (`shared/utils/duplicate-match.ts`, moved from `web/src/utils/duplicateMatch.ts`) against the promoted wines, for clients that can't run the TypeScript in-process — the iOS app. Free (local DB read). It exists so there is still exactly one definition of wine identity; do not port `scoreMatch` or `findDuplicate` to Swift.
...
Serper.dev (organic + Shopping) + Puppeteer. Populates a single `price_data` JSON column (`price_min`/`price_avg`/`price_max`/`retailers`/`nearest_retailer` are keys inside it, not separate SQL columns) plus `retailer_links`. A dead migration (`ws_price_min` etc., from an earlier Wine-Searcher-based design) exists in the DB but is never read/written by the adapter — don't build against it. **Noted 2026-09-04 (Phase 11):** `wines.appellation` (base `schema.sql`, not a migration) is a second, undocumented dead column — present in the table but absent from `WineRow`/`rowToWine`/`WineEntry` and never round-tripped by any route. Same treatment as the `ws_*` family: don't build against it, not worth a migration to remove yet. **Phase 9.3 note:** that same Wine-Searcher-era language had also leaked into live UI copy and code comments in `LabelScanFlow.tsx`, `WineCard.tsx`, and `WineDetailModal.tsx` — retired as part of Phase 9.3, since the actual price source has been Serper since this phase. Full detail in `docs/build-phases.md`.
...
Native iPhone app (SwiftUI), talking to the existing local backend over LAN. Key decisions (2026-09-21): label scanning uses the standard system camera picker (`UIImagePickerController`), not a custom capture UI; the Cellar tab is the dashboard, with no separate Home tab; manual "Add Wine" lives behind a Cellar-tab "+" and a scan-screen "Enter manually" link; the web app goes desktop-only (retiring Phase 11.1's 860px sidebar→top-bar collapse, moved to Phase 13); cloud hosting of the backend (for off-LAN access) is parked as its own future, unscoped phase rather than decided now. The original "no new endpoints" rule was amended at build start (2026-09-23) for two free backend additions — `POST /api/wines/duplicate-check` and the derived `latest_tasting_note_date` (see the schema bullets above) — rather than port wine identity to Swift or fetch notes per row. The iOS app lives in `ios/` (Xcode project, synchronized folders — new Swift files need no project-file edits); its Swift models hand-mirror `shared/types.ts`, and a change to that file must regenerate the iOS decoding fixtures with `backend/scripts/export-ios-fixtures.ts` in the same PR. Full detail in `docs/build-phases.md`.
...
| Environment monitoring | `modules/environment/` (not yet built) | Planned: SensorPush Cloud API → temperature + humidity readings. **Correction (Phase 11, 2026-09-04):** this row previously read as shipped; no such directory, route, or "SensorPush" reference exists anywhere in `backend/`, `shared/`, or `web/` today. Scoped to Phase 13 (`docs/build-phases.md`; was Phase 12 before the 2026-09-20 renumbering inserted the iOS build). |
...
The `GOOGLE_SHEETS_*` variables are Phase 1–4 only. They can be left empty from Phase 5 onward. `SERPER_API_KEY` is used by both `modules/price/` (Shopping endpoint) and `modules/reviews/` (organic search endpoint, Phase 7) — one key covers both. **`SENSORPUSH_EMAIL`/`SENSORPUSH_PASSWORD` are not read by any code yet** (Phase 11, 2026-09-04) — `modules/environment/` doesn't exist (see §5). Listed here for the Phase 13 module that will consume them; safe to leave empty until then.
...
- **Phase 12 capture surface:** the native iOS system camera picker (`UIImagePickerController`, decision D1 2026-09-21 — not a custom AVFoundation UI). The image is resized to 1024px **on device** before upload. The backend module does not change.
...
- Use Jest for the backend, Vitest for web; XCTest for iOS (`ios/WineAppTests/`, run from Xcode or `xcodebuild test` — **not yet in CI** as of 2026-09-23; add a macOS job once the project has compiled at least once)
...
### Pull requests
Every phase or discrete unit of work is delivered as a pull request. Title follows `<type>: <description>`. Description covers what changed, why, and decisions made. Link the relevant `docs/build-phases.md` phase. All CI checks must pass before merge. Squash merge. Claude Code opens the PR but does not merge it — the developer merges (or "rubber-stamps") it in GitHub. This applies even when the developer has explicitly asked in-session for a PR to be merged: open it, get CI green, then hand it back rather than clicking merge.

**A branch's PR must track what's actually on the branch — added 2026-09-16, after a real gap.** `feature/phase-11-ui-backend-integration`'s PR #31 merged on 2026-09-05, but the branch kept receiving commits afterward (design-system application, fixes, and eventually all of Phase 11.1) with no new PR ever opened — that work sat unmerged into `main` for 12 days until the developer asked directly. A PR reflects the branch's state *at the moment it's opened*; it does not track commits added after. So: **when a branch a PR was opened against gains new commits — because a session continued past that PR, or a later session reused the branch — open a new PR against `main` for the unmerged remainder before ending the session**, rather than assuming the earlier PR still covers it. Don't leave a session with unmerged branch work that no open PR currently represents.

**Corollary, same day:** this exact failure mode recurred at a smaller scale immediately — a doc-only commit landed on this section *after* PR #33 was opened, and the developer merged #33 before GitHub's PR view had caught up with that last push, so the commit was squashed out and had to be reapplied directly to `main` after the fact. When a PR gains a commit after it's opened, don't assume a merge that happens shortly after will pick it up — verify the merge commit's tree actually contains it (a direct two-dot tree diff, `git diff main <branch>`, not the three-dot/merge-base form) rather than trusting the PR page's displayed head.
